...
class LLVMBuilder(LLVMInterfaces.IToLLVM):
    ...
    def compute_expression(self, ast: ASTs.AST, force_boolean_result: bool = False):
        ...
        if isinstance(ast, ASTs.ASTBinaryExpression):
            ...
            raise NotImplementedError(f"The given expression {ast.get_content()} cannot be computed in LLVM yet")
    ...
    def declare_global_variable(self, ast: ASTs.ASTVarDeclaration):
        ...
        resulting_register = self.get_global_container().get_new_register(
            DataType.DataType(ast.get_data_type().get_token(), ast.get_data_type().get_pointer_level() + 1),
            ast.get_var_name())
        self.get_last_symbol_table().insert_variable(ast.get_var_name(), resulting_register)
        self.get_global_container().global_registers[resulting_register] = None
    ...
    def declare_and_init_global_variable(self, ast: ASTs.ASTVarDeclarationAndInit):
        ...
        self.get_global_container().add_variable_declaration_and_init(ast.get_var_declaration().get_var_name(),
                                                                      ast.get_data_type(), value_to_store.get_value())
        resulting_register = self.get_global_container().get_new_register(
            DataType.DataType(ast.get_data_type().get_token(), ast.get_data_type().get_pointer_level() + 1),
            ast.get_var_declaration().get_var_name())
        self.get_last_symbol_table().insert_variable(ast.get_var_declaration().get_var_name_ast().get_content(),
                                                     resulting_register)
        self.get_global_container().global_registers[resulting_register] = ast.initial_value
