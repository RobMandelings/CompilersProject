        ...
        for i in range(0, len(printf_elements)):
            ...
            if not string_element == '%':
                ...
                if arg_type == DataType.DataTypeToken.INT:
                    load_syscall_instruction = MipsInstruction.LoadImmediateInstruction(MipsValue.MipsRegister.V0, MipsValue.MipsLiteral(1))
                elif arg_type == DataType.DataTypeToken.FLOAT:
                    load_syscall_instruction = MipsInstruction.LoadImmediateInstruction(MipsValue.MipsRegister.V0, MipsValue.MipsLiteral(2))
                else:
                    raise NotImplementedError
